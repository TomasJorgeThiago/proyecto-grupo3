...
def todos_datos():
    #datos = diccionario con listas vacias de todas las columnas "importantes" 
    #que vamos a utilizar dentro de todo el programa, cada vez que
    #necesitemos llamar a los datos del archivo, llamaremos directamente
    #a "datos" sin abrir de vuelta el csv. por ejemplo:
    #print (datos["precio"]) y mostrara todos los precios
    #lectura= lo usamos para leer el csv, utilizamos el modulo: csv, 
    #utilizamos DictReader para poder leer el archivo usando los nombres
    #de las columnas
    #for row in lectura: recorre todas las filas del csv, por cada iteracion
    #  se guarda una fila distinta en cada columna o row
    #También verificamos que la latitud y longitud no estén vacías, 
    # ya que algunas estaciones no tienen coordenadas y no podrían
...
def contar_combustible(datos, id_combustible):
    ...
    #Recibe id_combustible y cuando encuentra una coincidencia, suma 1 al 
    # contador
...
def seleccionador_provincia(datos):
    #Esto es parte de todo el mapa, sirve para seleccionar en que provincia 
    # queremos visualizar
    # las empresas, el combustible, y el promedio de precios por tipo de combustible
    #Utiliza set() para eliminar provincias repetidas y sorted()para ordenarlas 
    # alfabéticamente.
...
def seleccionador_empresa(datos):
    #Esto es parte de todo el mapa, sirve para seleccionar que empresa 
    # queremos buscar, se puede
...
def seleccionador_combustible(datos):
    #Esto es parte de todo el mapa, sirve para seleccionar que tipo de 
    # combustibles queremos buscar
    #se puede acompañar por la provincia y por la empresa para tener 
    # datos mas exactos
...
def mapa_interactivo(datos,provincia, empresa, combustible):
    ...
    #Permite filtrar por provincia, empresa y tipo de combustible
    # (Gracias a los seleccionadores)
    #Recorre todas las estaciones del archivo y compara los datos 
    # con los filtros seleccionados por el usuario:
    #provincia, empresa y combustible.
    #el st.warning nos indica que si no encuentra estaciones que coincidan 
    # con los filtros,muestra un mensaje de advertencia.
...
def promedio_combustible(datos, provincia, id_combustible):
    ...
    #Recorre todas las estaciones y suma los precios solamente cuando 
    # coinciden con la provincia y el combustible seleccionado.
    #cuenta la cantidad de estaciones encontradas para poder 
    # realizar el promedio.
    #Si encuentra estaciones devuelve el promedio del precio. 
    # En caso contrario devuelve 0 para evitar errores.
    total = 0
    cantidad = 0
    for x in range(len(datos["provincia"])):
        if datos["provincia"][x] == provincia or provincia == "TODO":
...
def mostrar_promedios(datos, provincia):
    ...
    comb_nombre = [("2", "Nafta Super"), ("3", "Nafta Premium"), ("6", "GNC"), 
                   ("19", "Gasoil G2"), ("21", "Gasoil G3")]
...
def grafico_gnc_barato(datos):
    ...
    #cantidad_empresa:almacena la cantidad de estaciones de GNC 
    # que tiene cada empresa.
    #Recorre todas las estaciones y solamente toma aquellas cuales 
    # idproducto corresponde al GNC.
    #calcula el promedio dividiendo la suma de preciosentre la 
    ...
    for empresa in suma_empresa:
        promedio_empresa[empresa] = round(
            suma_empresa[empresa]/cantidad_empresa[empresa], 2)
